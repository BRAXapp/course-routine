...
function showGhostForSection(section){
  ...
  // hide solid blocks of this course
...
function tryToggleAdd(section){
  ...
  if(existingIndex >= 0){
    // check exam conflict with others (excluding this same course)
    ...
    // check time conflict with others (excluding this same course)
    const blocks = getEventBlocksForSection(section);
  ...
  // New course being added (not replacement)
...
function formatExam(date,start,end){
  ...
  const day=d.getDate();
  return `<span class="exam-date"><b>${month} ${day}</b></span> 
          <span class="exam-time"><b>${start}</b> - <b>${end}</b></span>`;
}
